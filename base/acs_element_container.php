...
*/
abstract class acs_element_container extends acs_element {
    ...
    */
    public function addElement($element_type,$element_ref,$element_label = null) {
        ...
        switch ($element_type) {
            case 'input':
                $newelement = new acs_form_input($element_ref);
            break;
            case 'select':
                $newelement = new acs_form_select($element_ref);
            break;
            case 'textarea':
                $newelement = new acs_form_textarea($element_ref);
            break;
            case 'fieldset':
                $newelement = new acs_fieldset($element_ref,$element_label);
                $nolabel = true;
            break;

            default:
                throw new acs_exception('Unknown element');
        }
    ...
    */
	public function addGroup($groupname, $type, array $values, $label = null) {
		$container = new acs_div($groupname . '_div');

		$container->setProcessWrap();

		if ($label)
			$container->add(new acs_form_label(null, $label));
    ...
    *
    * @param mixed $element Can either be pure html, acs_element or acs_element_container
    */
    public function add($element, $positonToAdd = null) {

        $ref = null;
        if (is_object($element)) { //assume acs_element descendent
    ...
    */
    protected function processElementsWrap($container_name) {
        ...
        if ($this->_wrapper == 'div')
            $wrapper = new acs_wrapper_div($container_name);
